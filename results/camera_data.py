...
class CameraData(Data):
    ...
    def deserialise_as_png(b):
        ...
        # As PNG, get from ID
        nir = cv2.imread(os.path.join(OUT_DIR, "nir", str(load_id) + "_nir.png"), \
                         cv2.IMREAD_ANYCOLOR)
        vis = cv2.imread(os.path.join(OUT_DIR, "vis", str(load_id) + "_vis.png"), \
                         cv2.IMREAD_ANYCOLOR)
    ...
    def contrast(self):
        ...
        result *= ((out_max - out_min) / (
                in_max - in_min))  # Divide away input range and then multiply in output range
        result += in_min  # N
